...
def nice_picture(logs, reinforcers, x_solution, S_solution, t_eval):
    """
    ...
    for state_id in range(num_states):
        for action_id in range(num_actions):
            ...
            axes[0, 0].plot(
                range(len(q_values_logs)), q_values_logs,
                linestyle=action_styles[action_id],
                color=state_styles[state_id % len(state_styles)],
    ...
    for player_id, reinforcer in enumerate(reinforcers):
        for state_id in range(num_states):
            ...
            axes[1, 0].plot(
                range(len(probabilities_logs)), probabilities_logs,
                color=state_styles[state_id % len(state_styles)],
                linestyle=player_styles[player_id],
                alpha=0.4, linewidth = 3
            )
    ...
    for state_id in range(num_states):
        ...
        axes[1, 1].plot(
            range(len(state_proportions_logs)), state_proportions_logs,
            color=state_styles[state_id % len(state_styles)],
            alpha=0.4
        )
